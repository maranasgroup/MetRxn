...
Whenever there are multiple canonical associations, for metabolites, the first tab shows up and dissappears.
...
for($i = 1; $i <= count($tabArray); $i++)
{
if($type == "metabolites")
	{
		echo "document.getElementById(\"tab".$i."\").className = \"hideM\";\n";
		echo "document.getElementById(\"image".$i."\").className = \"\";\n";
	}
if($type == "reactions")
	{
		echo "document.getElementById(\"tab".$i."\").className = \"\";\n";
		echo "document.getElementById(\"image".$i."\").className = \"hideR\";\n";
	}
...
echo "document.getElementById(\"tab".$tabArray[$cd_hash]."\").className = \"active\";\n";
echo "document.getElementById(\"image".$tabArray[$cd_hash]."\").className = \"show\";\n";
...
$tabQuery1 = "select group_concat(cd_hash) cd_hashGrp from (select cd_hash,count(distinct idSource) cnt from `metrxn`.`abbrsynstrsrcview` A
		where syn = '$var' $srcQueryAppend
		and type = '$type'
		and idSource in (select distinct A.idSource from `metrxn`.`source_user_pin` A,`metrxnweb`.`login` B
		where A.idUser = B.idlogin and B.username = '".$username."')
		group by cd_hash) A  
		group by cnt order by cnt desc limit 1;";

		
$resulttabQuery1 = mysql_query($tabQuery1);

while($rowtabQuery1 = mysql_fetch_array($resulttabQuery1))
{
	$tabQuery2 = "select abs(cd_hash) id,cd_hash,count(distinct idSrc) cnt
			from `metrxn`.`abbrsynstrsrc`
			where cd_hash in (".$rowtabQuery1['cd_hashGrp'].")
			group by cd_hash order by cnt desc limit 1;";
}


//echo $tabQuery2;
		
$tabQuery = "select * from(SELECT distinct syn,abs(A.cd_hash) id,cd_hash,count(distinct source) cnt,count(distinct idSrc) cnt2  
...
echo "Time taken for tab query = $total_exec_time";
echo "<br></br>"; */
//echo $tabQuery;
...
if($resultCount >= 1)		
{
	...
	{	
		...
		echo "<li onclick=\"makeactive(".$count.")\"><a class=\"\" id=\"tab".$count."\">".$var."</a></li>\n";
		if($count ==1)
		{
			/*make the first tab active */
			echo "<body onload=\"makeactive(".$count.")\">";
...
while($row = mysql_fetch_array($result))
{
	echo "<div class=\"hide\" id=\"image".$tabArray[$row['cd_hash']]."\">\n";
	echo "<table>";	
	
	if(isset($_GET['src']))
	{
		/*only reaction results come here*/
		$src = $_GET['src'];
		if($src)
		{
			...
			echo "Time taken for As it appears query = $total_exec_time";
			echo "<br></br>"; */
			...
			echo "Time taken for Abbreviations in reactions query = $total_exec_time";
			echo "<br></br>"; */
			/*time*/
...
/*			echo "mview_param('mol', '../StructureFile/".$row9['cd_id'].".".$fileType."');\n";*/
	...
	$smilesQuery = "select A.cd_smiles smiles,
	...
	$result2 = mysql_query($smilesQuery);
	...
	/*time */
			...
			echo "Time taken for SMILES query = $total_exec_time";
			echo "<br></br>"; */
	/*time*/
