...
/*			echo "mview_param('mol', '../StructureFile/".$row9['cd_id'].".".$fileType."');\n";*/
	...
	$time_end = microtime(true);
	$total_exec_time = $time_end - $time_start;
	echo "Time taken for the smiles query $total_exec_time";
	...
	while($row2 = mysql_fetch_array($result2))
	{
		...
		$time_end = microtime(true);
		$total_time_exec = $time_end - $time_start;
	echo "time taken for the other metabolties query $total_time_exec";	
	...
	echo $queryReactionParticipation;
		...
		echo "time take for the reaction participation query = $total_exec_time";
		echo "</table>";
		echo "</table>";/*for the text before the table*/
